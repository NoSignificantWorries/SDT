...
for i in range(len(main.data)):
    ...
    for j, group in enumerate(main.classes):
        ...
        if (i // main.params_count) != (i % main.params_count):
            ...
            model_restricted = sm.OLS(y, np.ones((local_data.shape[0], 1))).fit()
            ...
            lr_statistic = 2 * (model_full.llf - model_restricted.llf)
            lr_df = model_full.df_model
            lr_pvalue = 1 - scipy.stats.chi2.cdf(lr_statistic, lr_df)
            
            # Score test
            score_statistic = model_full.compare_lr_test(model_restricted)[0]
            score_df = model_full.df_model
            score_pvalue = 1 - scipy.stats.chi2.cdf(score_statistic, score_df)
            
            # Regression
            intercept = model_full.params[0]
            slope = model_full.params[1]
            ...
            results.append({
                ...
                'F_df_num': f_df_num,
                'F_df_denom': f_df_denom,
                'F_pvalue': f_pvalue,
                'LR_chi2': lr_statistic,
                'LR_df': lr_df,
                'LR_pvalue': lr_pvalue,
                'Score_chi2': score_statistic,
                'Score_df': score_df,
...
print("\nResults Table:")
print("=" * 150)
print(f"{'Класс':<10} {'X':<15} {'Y':<15} {'Формула':<40} {'R²':<8} {'F-pval':<8} {'LR-pval':<8} {'Score-pval':<8}")
print("-" * 150)

for idx, row in res_df.iterrows():
    formula_short = f"{row['Slope']:.2f}*X + {row['Intercept']:.2f}"
    print(f"{row['Class']:<10} {row['X_variable']:<15} {row['Y_variable']:<15} "
          f"{row['Formula'][:38]:<40} {row['R_squared']:.4f}  {row['F_pvalue']:.4f}   "
          f"{row['LR_pvalue']:.4f}    {row['Score_pvalue']:.4f}")

res_df.to_csv('res/regression_statistics.csv', index=False, encoding='utf-8-sig')
